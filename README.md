...
| 지표                        | 정의                                | 수식                                     | 비고                   |   |                   |
| ------------------------- | --------------------------------- | -------------------------------------- | -------------------- | - | ----------------- |
| **MTF50(px)**             | Slanted-edge의 50% contrast 응답 해상도 | SFR(f) = 0.5인 주파수 (cy/pixel)           | `slanted-edge` 방식 구현 |   |                   |
| **SNR(dB)**               | 균일 영역에서의 신호 대 잡음비                 | SNR = 20·log₁₀(μ/σ)                    | μ: 평균 밝기, σ: 표준편차    |   |                   |
| **ΔL 플리커(%)**             | frame-to-frame 밝기 요동률             | ΔL = (Lmax − Lmin)/(Lmax + Lmin) × 100 | flicker 감지용          |   |                   |
| **Sharpness(LapVar)**     | 라플라시안 기반 디테일 보존도                  | var(∇²I)                               | edge clarity 추정 지표   |   |                   |
| **Motion Blur Width(px)** | 움직이는 edge의 10–90% transition 폭    | w = x90% − x10%                        | 이동시 에지 전이 길이         |   |                   |
...
## 8-1. 비교 UI 설계 스케치
```
┌─────────────── Camera Tuner Compare ───────────────┐
│ [CAM1 ▼] [Start] [Stop]       [CAM2 ▼] [Start]     │
│────────────────────────────────────────────────────│
│ ┌────────────── Live View 1 ─────────────┐ ┌────────────── Live View 2 ─────────────┐ │
│ │                                       │ │                                       │ │
│ │         (실시간 스트림 - 좌측)        │ │         (실시간 스트림 - 우측)        │ │
│ └───────────────────────────────────────┘ └───────────────────────────────────────┘ │
│ ┌────────────── Metrics 1 ──────────────┐ ┌────────────── Metrics 2 ──────────────┐ │
│ │ MTF50(px):        2.5                │ │ MTF50(px):        1.8                │ │
│ │ SNR(dB):          29.3               │ │ SNR(dB):          26.1               │ │
│ │ MotionBlur(px):   3.2                │ │ MotionBlur(px):   7.4                │ │
│ │ LapVar:           102.4              │ │ LapVar:           66.8               │ │
│ │ ΔL Flicker(%):    1.9                │ │ ΔL Flicker(%):    6.3                │ │
│ └──────────────────────────────────────┘ └───────────────────────────────────────┘  │
│────────────────────────────────────────────────────────────────────────────────────│
│ [ Snapshot Both ]   [ Export Report ]   [ Save Metrics.csv ]                      │
└────────────────────────────────────────────────────────────────────────────────────┘
```
